...
namespace SK.Tekla.Drawing.Automation.Services
{
    public class BeamDrawingCreator
    {
        ...
        private SKGussetDimension gussetDimension;
       ...
       log4net.LogManager.GetLogger(typeof(BeamDrawingCreator));
                    ...
                    try
                    {
                        ...
                        while (enum_for_views.MoveNext())
                        {
                            ...
                            if (current_view.ViewType.Equals(TSD.View.ViewTypes.FrontView))
                            {
                                ...
                                CreateBottomView(mymodel, main_part, output, BOTTOM_VIEW_TOCREATE, defaultADFile, list_of_parts_for_bottom_view_mark_retain, ref bottom_view_list, ref bottom_view_FLANGE_CUT_LIST,
                                    MAINPART_PROFILE_VALUES, main, p1_bottm, p2_bottm, current_view, SHORTNENING_VALUE_FOR_BOTTOM_VIEW);
                                #endregion
                                #region top_view_creation

                                TOP_view_FLANGE_CUT_LIST = CreateTopView(type_for_bolt, output, TOP_VIEW_TOCREATE, TOP_VIEW_needed, defaultADFile, MAINPART_PROFILE_VALUES, main, p3_bottm, p4_bottm, fixed_attributes, current_view);
                            ...
                            if (current_view.ViewType.Equals(TSD.View.ViewTypes.TopView))
                            {
                                ...
                                ConfigureTopViewBoltDimensions(mymodel,
                        ...
                        if (list.Count > 0)
                        {
                            ...
                            SectionView(mymodel, my_handler, defaultADFile, main_part, list,
                                SCALE, MAINPART_PROFILE_VALUES, fixed_attributes,
                                catalog_values, dim_font_height1, sectionviews_in_drawing);
                        }
                        ...
                        if (list_section_flange.Count > 0)
                        {
                            ...
                            SectionFlange(mymodel, my_handler, defaultADFile, main_part,
                                list_section_flange, SCALE, MAINPART_PROFILE_VALUES,
                                fixed_attributes, catalog_values, dim_font_height1, sectionviews_in_drawing);
                        }
                        #endregion
        ...
        private void SectionView(Model mymodel, DrawingHandler my_handler,
            string defaultADFile, TSM.Part main_part,
            List<SectionLocationWithParts> list, double SCALE,
            List<double> MAINPART_PROFILE_VALUES,
            StraightDimensionSet.StraightDimensionSetAttributes fixed_attributes,
            List<double> catalog_values,
            StraightDimensionSet.StraightDimensionSetAttributes dim_font_height1,
            List<TSD.View> sectionviews_in_drawing)
        {
            ...
            for (int z = 0; z < list.Count; z++)
            {
                ...
                if (current_view != null)
                {
                    ...
                    foreach (var list_of_parts in list[z].RequiredPartList)
                    {
                        ...
                        if (!mypart.Name.Contains("STUD") && !mypart.Equals(main_part) && !mypart.Name.Contains("GUSSET"))
                        {
                            ...
                            if ((Convert.ToInt64(mypt[0].Y) >= h1) && (Convert.ToInt64(mypt[1].Y) >= h1))
                            {
                                ...
                                if (s > 0)
                                {
                                    ...
                                    if ((result_for_check_for_bolt_in_part) && (prof_type != "L"))
                                    {
                    ...
                    foreach (var list_of_parts in list[z].RequiredPartList)
                    {
                        ...
                        while (enum_for_mark.MoveNext())
                        {
                            ...
                            if (mark.GetType().Equals(typeof(TSD.Mark)))
                            {
                                ...
                                while (enumcheck.MoveNext())
                                {
                                    ...
                                    if (mark_part.GetType().Equals(typeof(TSD.Part)))
                                    {
                                        ...
                                        if (list[z].RequiredPartList.Any(p => p.Identifier.ID == modelpart.Identifier.ID))
                                        {
                                        }
                                        else
                                        {
                            ...
                            else if (mark.GetType().Equals(typeof(TSD.WeldMark)))
                            {
                                ...
                                if ((list[z].RequiredPartList.Any(p => p.Identifier.ID == mainpart.Identifier.ID)) || ((list[z].RequiredPartList.Any(p => p.Identifier.ID == secondary_part.Identifier.ID))))
                                {
        ...
        private void SectionFlange(Model mymodel, DrawingHandler my_handler, string defaultADFile, TSM.Part main_part, List<SectionLocationWithParts> list_section_flange, double SCALE, List<double> MAINPART_PROFILE_VALUES, StraightDimensionSet.StraightDimensionSetAttributes fixed_attributes, List<double> catalog_values, StraightDimensionSet.StraightDimensionSetAttributes dim_font_height1, List<TSD.View> sectionviews_in_drawing)
        {
            /////////////////////////////////////////////////bolt gage dimension in section view using matrix function /////////////////////////////////////////////////////////////
...
SKSortingHandler.SortOrder.Descending);
        ...
        private List<TSD.View> CreateTopView(Type type_for_bolt, double output, string TOP_VIEW_TOCREATE,
            string TOP_VIEW_needed, string drg_att, List<double> MAINPART_PROFILE_VALUES, Beam main, TSG.Point p3_bottm, TSG.Point p4_bottm, StraightDimensionSet.StraightDimensionSetAttributes fixed_attributes, TSD.View current_view)
        {
            List<TSD.View> TOP_view_FLANGE_CUT_LIST = new List<TSD.View>();
